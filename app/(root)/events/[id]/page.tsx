...
const EventDetails = async ({ params: { id }, searchParams}: SearchParamProps) => {
  ...
  return (
    <>
          ...
          <div className='flex w-full flex-col gap-8 p-5 md:p-10'>
            <div className='flex flex-col gap-6'>
              ...
              <div className='flex flex-col gap-3 sm:flex-row sm:items-center'>
                <div className='flex gap-3'>
                  ...
                  <p className='p-medium-16 rounded-full px-4 py-2.5 text-grey-500 bg-grey-500/10'>{event.category.name}</p>
                </div>
            ...
            <div className='flex flex-col gap-5'>
              <div className='flex gap-2 md:gap-3'>
                <Image src={calendarLogo} alt="calendar" width={32} height={32}/>
                <div className='p-medium-16 lg:p-regular-20 flex flex-wrap items-center'>
                  <p>
                    {formatDateTime(event.startDateTime).dateOnly} - {" "} {formatDateTime(event.startDateTime).timeOnly}
                  </p>
                  <p>
                    {formatDateTime(event.startDateTime).dateOnly} - {" "} {formatDateTime(event.endDateTime).timeOnly}
                  </p>
                </div>
              </div>
      ...
      {/* Recommended Posts - Events with the same category */}
